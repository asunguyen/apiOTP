...
$(document).ready(function() {
  ...
  async function getInfoUser() {
    $(".loading-wp").show();
    let res = await api.get("user/get-info");
    if (res && res.code == 200) {
      localStorage.setItem("user", JSON.stringify(res.data));
      localStorage.setItem("totalUser", JSON.stringify(res.total));
      $(".amount-account").text(res.data.amount + " vnđ");
      $("#username-account").text(res.data.username);
      if (!res.data.isAdmin) {
